...
# [개선] 정규분포 그래프: 아웃라이어 제거 로직 및 타입별 한계선 반영
def plot_bell_curve(ax, data_series, selected_values, title, mic_type):
    # 1. 아웃라이어 제거 (관리 범위 주변의 시료만 통계에 반영하여 왜곡 방지)
    raw_vals = pd.to_numeric(data_series, errors='coerce').dropna()
    if mic_type == 'analog':
        lcl, ucl = -11, -9
        clean_data = raw_vals[(raw_vals > -20) & (raw_vals < 0)]
    else:
        lcl, ucl = -38, -36
        clean_data = raw_vals[(raw_vals > -45) & (raw_vals < -25)]
    
    if len(clean_data) < 2: 
        ax.text(0.5, 0.5, "Insufficient Data in Spec Range", ha='center')
        return
    
    mu, std = clean_data.mean(), clean_data.std()
    # X축 범위를 관리 한계선 기준으로 고정하여 보기 좋게 설정
...
if uploaded_file is not None:
    ...
    if selected_indices and show_dist_plot:
        ...
        for i, ch in enumerate(config["channels"]):
            ...
            plot_bell_curve(dist_axes[i], all_1k, sel_1k, f"{ch['name']} - 1kHz Sensitivity Distribution", ch["type"])
        plt.tight_layout()
        st.pyplot(dist_fig)
