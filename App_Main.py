...
FIG_WIDTH = 12       # 그래프 가로 너비 통일
PLOT_HEIGHT = 6      # 채널당 세로 높이
FONT_SIZE_TITLE = 16 # 제목 크기
FONT_SIZE_AXIS = 12  # 축 및 눈금 크기
...
def classify_sample(row, cols, freqs, limit_low, limit_high):
    ...
    l_low, l_high = pd.to_numeric(limit_low[cols], errors='coerce'), pd.to_numeric(limit_high[cols], errors='coerce')
    is_fail = (val < l_low) | (val > l_high)
...
def get_row_summary_data(row, ch_info, all_cols):
    ...
    if thd_idx is not None:
        try: data[th_key] = f"{float(row[all_cols[thd_idx]]):.3f}"
        except: data[th_key] = "-"
...
def create_fr_plot(config, df, current_test_data, limit_low, limit_high, show_normal, plotting_normal_indices, highlight_indices):
    ...
    for i, ch in enumerate(config["channels"]):
        ...
        ax.minorticks_off()
        if show_normal:
            for n in plotting_normal_indices: 
        ...
        ax.set_ylim(ylim)
        ax.grid(True, which='major', linestyle='-', alpha=0.4, color='#bdc3c7')
    plt.tight_layout()
    return fig

def plot_bell_curve(ax, data_series, stats_indices, selected_indices, title, mic_type):
    stats_data = pd.to_numeric(data_series.iloc[stats_indices], errors='coerce').dropna()
    lcl, ucl = (-11, -9) if mic_type == 'analog' else (-38, -36)
    if len(stats_data) < 2: return
    mu, std = stats_data.mean(), stats_data.std()
    ...
    ax.plot(x, p, 'k', lw=2.5, alpha=0.7)
    ...
    stats_txt = f"Sample N: {len(stats_data)}\nMean: {mu:.2f}\nStd: {std:.3f}\nCpk: {cpk:.2f}"
    ...
    ax.tick_params(axis='both', which='major', labelsize=FONT_SIZE_AXIS)
    ax.set_xlim(x_min, x_max); ax.legend(loc='upper right', fontsize=FONT_SIZE_AXIS - 2)
...
if uploaded_file:
    ...
    df = pd.read_csv(io.StringIO(raw_bytes.decode(chardet.detect(raw_bytes)['encoding'], errors='replace')), low_memory=False)
...
if df is not None:
    ...
    test_data = raw_test_data[raw_test_data[sn_col].str.contains('/', na=False)].reset_index(drop=True)
    ...
    if len(test_data) > 0:
        ...
        for idx, row in test_data.iterrows():
            is_any_fail, is_stats_eligible, is_pure_normal = False, True, True
            row_table = []
            for ch in config["channels"]:
                ...
                if status == "Defect": is_stats_eligible, is_pure_normal = False, False
                if status == "Margin Out": is_pure_normal = False
                if status != "Normal": is_any_fail = True
                ...
                # 상세 테이블용 데이터 구성 (함수 호출로 통일)
                row_res = get_row_summary_data(row, ch, df.columns)
                row_res["Status"] = status
                row_table.append(row_res)

            sample_info[idx] = {"table": pd.DataFrame(row_table), "sn": str(row[sn_col]).strip()}
            if is_any_fail: issue_indices.append(idx)
            if is_stats_eligible: stats_indices.append(idx)
        ...
        with d2:
            ...
            st.markdown(f"""<div style="display: flex; gap: 8px; margin-bottom: 5px;">
                <div style="background-color: #f8f9fa; padding: 10px 15px; border-radius: 10px; border-left: 5px solid #28a745; flex: 1;">
                <p style="margin:0; font-size:11px; color:#6c757d; font-weight:bold;">PASS</p><p style="margin:0; font-size:24px; font-weight:800; color:#28a745;">{total_pass}</p></div>
                <div style="background-color: #f8f9fa; padding: 10px 15px; border-radius: 10px; border-left: 5px solid #dc3545; flex: 1;">
                <p style="margin:0; font-size:11px; color:#6c757d; font-weight:bold;">FAIL</p><p style="margin:0; font-size:24px; font-weight:800; color:#dc3545;">{total_fail}</p></div></div>""", unsafe_allow_html=True)
        ...
        with d3:
            ...
            for ch_name, stat in ch_stats.items():
                v = np.array(stat["vals_1k"])[stats_indices]
                ch_summary.append({"Channel": ch_name, "Pass": stat["pass"], "Fail": stat["fail"], "Yield": f"{(stat['pass']/total_qty)*100:.1f}%", "Min": f"{v.min():.2f}", "Max": f"{v.max():.2f}", "Avg": f"{v.mean():.2f}", "Stdev": f"{v.std():.2f}"})
        ...
        if sel_idx and show_detail_table:
            st.info("🔍 **선택 시료 상세 분석 테이블**")
            for i in sel_idx:
                st.write(f"📄 **SN: {sample_info[i]['sn']}**")
        ...
        if show_dist_plot:
            ...
            for i, ch in enumerate(config["channels"]):
                plot_bell_curve(axes_d[i], test_data[df.columns[ch["range"]][np.argmin(np.abs(np.array(get_freq_values(df.columns[ch["range"]])) - 1000))]], stats_indices, sel_idx, f"{ch['name']} - Distribution", ch["type"])
            st.pyplot(fig_d)
else:
    st.info("사이드바에서 CSV 로그 파일을 업로드하세요.")